...
export default function AdminPage() {
  ...
  async function load(event: FormEvent) {
    ...
    try {
      const [o, t] = await Promise.all([listManagerOrders(key), listManagerSupport(key)]);
  ...
  async function setTicketStatus(id: number, status: SupportStatus) {
    try { const updated = await updateManagerSupportStatus(id, status, key); setTickets((prev) => prev.map((t) => (t.id === id ? updated : t))); }
  ...
  return <main className="min-h-screen bg-[var(--color-bg-soft)] px-5 py-12 text-[var(--color-text)]"><div className="mx-auto max-w-5xl">
    <h1 className="text-3xl font-bold">Панель оператора</h1><p className="mt-2 text-sm text-[var(--color-text-muted)]">Заявки, обращения в поддержку и повторная отправка подтверждений. Ключ не сохраняется в браузере.</p>
